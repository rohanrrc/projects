...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def computeQValueFromValues(self, state, action):
        ...
        "*** YOUR CODE HERE ***"
        total = 0
        stp = self.mdp.getTransitionStatesAndProbs(state,action)
        for s, prob in stp:
            total = total + prob*(self.mdp.getReward(state,action,s) + self.discount*(self.values[s]))
        return total
    ...
    def computeActionFromValues(self, state):
        ...
        for action in a:
            stp = self.mdp.getTransitionStatesAndProbs(state,action)
            for s, prob in stp:
                total[action] = total[action] + prob*(self.mdp.getReward(state,action,s) + self.discount*self.values[s])
        return total.argMax()
