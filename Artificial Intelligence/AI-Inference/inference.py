...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        captured = util.Counter()
        captured[self.getJailPosition()] = 1

        if noisyDistance == None:
            allPossible = captured
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                newCounter[newPos] = newCounter[newPos] + self.beliefs[oldPos]*prob
...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            self.particles = [util.sample(allPossible) for i in range(0,self.numParticles)]
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        "*** YOUR CODE HERE ***"
        self.particles = []
        perms = list(itertools.permutations(self.legalPositions, self.numGhosts))
        random.shuffle(perms)
        for i in range(0,self.numParticles):
            self.particles.append(tuple(perms[i%len(perms)]))
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                ...
                if p[i] != self.getJailPosition(i):
                    product = product * emissionModels[i][trueDistance]
            allPossible[p] = allPossible[p] + product
        ...
        for oldParticle in self.particles:
            ...
            for i in range(self.numGhosts):
                newPosDist = getPositionDistributionForGhost(setGhostPositions(gameState, newParticle),
                                                       i, self.ghostAgents[i])
                newpart.append(util.sample(newPosDist))
            newParticle = newpart
            "*** END YOUR CODE HERE ***"
